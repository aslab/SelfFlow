...
def generate_launch_description():
    ...
    self_flow_dir = get_package_share_directory('self_flow')
    bringup_dir = get_package_share_directory('nav2_bringup')
    launch_dir = os.path.join(bringup_dir, 'launch')

    # Names and poses of the robots
    robots = [
        {'name': 'robot1', 'x_pose': 0.0, 'y_pose': 0.5, 'z_pose': 0.01, 'model': 'waffle'},
        {'name': 'robot2', 'x_pose': 0.0, 'y_pose': -0.5, 'z_pose': 0.01, 'model': 'burger'}]
    ...
    declare_robot1_params_file_cmd = DeclareLaunchArgument(
        'robot1_params_file',
        default_value=os.path.join(self_flow_dir, 'params', 'nav2_multirobot_params_1.yaml'),
        description='Full path to the ROS2 parameters file to use for robot1 launched nodes')

    declare_robot2_params_file_cmd = DeclareLaunchArgument(
        'robot2_params_file',
        default_value=os.path.join(self_flow_dir, 'params', 'nav2_multirobot_params_2.yaml'),
        ...
        spawn_robots_cmds.append(
            IncludeLaunchDescription(
                ...
                launch_arguments={
                                  ...
                                  'turtlebot_type': TextSubstitution(text=str(robot['model']))
                                  }.items()))
